...
- [x] En modo ordenar **ningún widget dispara su función**: mover un botón no debe además pulsarlo. Eventos de puntero, no arrastre HTML5, que no existe en táctil.
- [x] **Arrastrar de verdad, y decir dónde va a caer antes de que caiga.** Ordenar era tocar y volver a tocar: no había nada siguiendo al dedo ni nada diciendo dónde iba a quedarse. Ahora un fantasma sigue al puntero y un cursor marca el hueco exacto entre qué dos widgets caería; se suelta y cae ahí. El gesto por toques sigue estando —es la manera precisa en un móvil, y la única sin puntero— y lo decide un umbral de 8 px, así que un toque que tiembla sigue siendo un toque. Escape lo devuelve todo.
  - **La planta**: la lámpara se quedaba quieta hasta soltarla y entonces saltaba. Ahora va con el dedo, lleva su posición en metros escrita encima mientras se mueve, y aparece una retícula solo mientras se coloca.
  - **La timeline**: el solape lo rechazaba el daemon *después* de soltar, y la barra volvía de un salto. Ahora la barra se pone roja y dice con qué choca mientras aún se puede mover, la barra de destino se marca, y soltar ahí no manda nada. El daemon sigue siendo quien decide; lo que ha cambiado es cuándo se entera uno.
  - **Modo editar**: una línea dice qué hace un toque ahora que ya no dispara el widget, tocar el fondo cierra el panel, Escape también, y en móvil el panel se trae a la vista — pulsar algo y que no pase nada visible es la forma más rápida de enseñarle a alguien que una interfaz está rota.
  - La aritmética del arrastre vive en `web/src/drag.ts`, aparte de los componentes y con 16 tests: un punto de inserción desviado en uno coloca la cue donde no era y parece culpa del operador.
...
- [x] **Modo operador y PWA instalable.** En modo operador queda la consola y nada más: los controles de edición y el conmutador de vistas **no están en el DOM**, no ocultos con una clase — un control que solo es invisible sigue ahí para una pulsación despistada. El modo es pegajoso, porque el móvil del que hablamos está pegado con cinta a un truss y lo reabre quien no lo puso ahí. Se sale manteniendo pulsado un segundo: un toque es lo que hace una manga, un segundo de presión deliberada no. **Es una defensa contra accidentes, no un candado**, y así se dice — decir lo contrario sería una afirmación que este código no puede sostener.
  - El service worker **no cachea nada de `/api` ni de `/ws`**. Esto es una mesa de luces: el daemon *es* la mesa, así que hacer que la app «funcione sin conexión» sería mentir justo donde importa que te crean. Lo que sí hace: la app se instala en la pantalla de inicio y abre sin barra de navegador, y el shell carga al instante en vez de por el wifi del local. El shell va *network-first*, así que una actualización del daemon se coge en la siguiente carga — una consola con el bundle de la semana pasada contra la API de esta semana falla de maneras que nadie sabe reproducir.
  - El icono lo copia el build desde el que ya usan el lanzador y el AppImage. Dos copias de un icono son dos iconos: el que se redibuja y el que deja de parecerse en silencio.
...
- [x] **Apariencia y control externo por widget.** Los colores ya se leían y se pintaban; lo que faltaba era poder cambiarlos, y eso es lo que convierte «lo veo» en «lo edito». Lo cosmético en una mesa no es decoración: una botonera de colores donde todo es gris es una botonera que nadie puede usar a oscuras, y el operador eligió esos colores por algo.
  - La tipografía viaja como la guarda el fichero — `QFont::toString()`, dieciséis campos — y solo se editan la familia y el cuerpo, así que los catorce que nadie toca sobreviven al viaje. Mandar la familia a secas sería una fuente sin tamaño, sin peso y sin estilo: algo que no eligió nadie.
  - `null` devuelve un color o una fuente al **Default** de la mesa, que no es lo mismo que elegir un gris que se le parezca: QLC+ escribe «Default» y cada tema lo pinta a su manera.
  - **El control externo se aprende pulsándolo.** Nadie sabe que su fader es el canal 47 del universo de entrada 1, y un formulario que pide dos números es un formulario que nadie rellena. El daemon reenvía cada entrada que ve, en su propio mensaje y sin coalescer, porque un control que ya ha cambiado cuando llega el mensaje es el control equivocado.
  - **Desasignar borra el elemento, no lo vacía.** Un `<Input>` sin atributos lo lee QLC+ como universo 0, canal 0: una asignación a algo real, y a lo que no es.

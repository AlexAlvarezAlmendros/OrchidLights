#!/bin/bash
#
# What an edit to the console does to the faders holding a look.
#
#   server/test/fader-smoke.sh [path-to-orchidlightsd] [path-to-project.qxw]
#
# Editing the Virtual Console makes the daemon re-read it and re-register
# everything. Both the value a fader is holding and its claim on its channels
# have to survive that -- and neither did: every edit stranded a fader that
...
"$DAEMON" --port "$PORT" --no-output "$WORK/$(basename "$PROJECT")" > /tmp/orchid-fader.log 2>&1 &
PID=$!
trap 'kill $PID 2>/dev/null || true; rm -rf "$WORK"' EXIT

for _ in $(seq 1 100); do
    curl -sf --max-time 1 "http://127.0.0.1:$PORT/api/v1/status" > /dev/null 2>&1 && break
    kill -0 $PID 2>/dev/null || { cat /tmp/orchid-fader.log >&2; fail "the daemon exited early"; }
    sleep 0.2
done
...
node "$HERE/fader-client.mjs" "ws://127.0.0.1:$PORT/ws" "$TOKEN" \
    || fail "an edit to the console broke the faders holding a look"

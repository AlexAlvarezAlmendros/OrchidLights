#!/bin/bash
#
# What an edit to the console does to the matrixs holding a look.
#
#   server/test/matrix-smoke.sh [path-to-orchidlightsd] [path-to-project.qxw]
#
# Editing the Virtual Console makes the daemon re-read it and re-register
# everything. Both the value a matrix is holding and its claim on its channels
# have to survive that -- and neither did: every edit stranded a matrix that
...
PROJECT=${2:-server/test/data/vc-matrix.qxw}
PORT=${PORT:-9963}
...
"$DAEMON" --port "$PORT" --no-output "$WORK/$(basename "$PROJECT")" > /tmp/orchid-matrix.log 2>&1 &
PID=$!
trap 'kill $PID 2>/dev/null || true; rm -rf "$WORK"' EXIT

for _ in $(seq 1 100); do
    curl -sf --max-time 1 "http://127.0.0.1:$PORT/api/v1/status" > /dev/null 2>&1 && break
    kill -0 $PID 2>/dev/null || { cat /tmp/orchid-matrix.log >&2; fail "the daemon exited early"; }
    sleep 0.2
done
...
node "$HERE/matrix-client.mjs" "ws://127.0.0.1:$PORT/ws" "$TOKEN" \
    || fail "a matrix widget did not do what its presets say"
